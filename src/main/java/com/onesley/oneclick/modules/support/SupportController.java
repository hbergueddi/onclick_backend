...
/**
 * Bug 32 (Batch D RBAC v2) — double-binding hasAnyRole(...) or hasAuthority('VERB:SUPPORT')
 */
@RestController
...
public class SupportController {
    ...
    @PreAuthorize("hasAnyRole('SUPERADMIN', 'GROUP_ADMIN', 'RESTAURATEUR') or hasAuthority('VIEW:SUPPORT')")
    ...
    @PreAuthorize("isAuthenticated() or hasAuthority('VIEW:SUPPORT')")
    public TicketDto findById(@PathVariable UUID id) { return service.findById(id); }

    @PostMapping("/tickets")
    @PreAuthorize("isAuthenticated() or hasAuthority('CREATE:SUPPORT')")
    ...
    @PreAuthorize("isAuthenticated() or hasAuthority('UPDATE:SUPPORT')")
