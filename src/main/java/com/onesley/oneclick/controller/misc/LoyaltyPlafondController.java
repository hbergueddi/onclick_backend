package com.onesley.oneclick.controller.misc;

import com.onesley.oneclick.dto.misc.LoyaltyPlafondDto;
import com.onesley.oneclick.service.misc.LoyaltyPlafondService;
import io.swagger.v3.oas.annotations.Operation;
import io.swagger.v3.oas.annotations.tags.Tag;
import java.util.List;
import java.util.UUID;
import org.springframework.http.ResponseEntity;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;

/**
 * REST controller pour {@link LoyaltyPlafondDto} (généré par scripts/scaffold-jpa.mjs).
 * Endpoints minimaux — étendre selon les besoins métier (filtres, pagination, mutations).
 *
 * <p>Sécurité par défaut : permitAll (groupe non classifié).
 * À raffiner endpoint par endpoint quand la business logic est portée (Phase 11+).
 */
@RestController
@RequestMapping("/api/loyalty-plafonds")
@Tag(name = "LoyaltyPlafond", description = "Auto-generated controller for loyalty_plafonds")
public class LoyaltyPlafondController {

    private final LoyaltyPlafondService service;

    public LoyaltyPlafondController(LoyaltyPlafondService service) {
        this.service = service;
    }

    @GetMapping
    @Operation(summary = "Liste tous les éléments")
    public List<LoyaltyPlafondDto> findAll() {
        return service.findAll();
    }

    @GetMapping("/{id}")
    @Operation(summary = "Détail d'une LoyaltyPlafond par UUID")
    public ResponseEntity<LoyaltyPlafondDto> findById(@PathVariable UUID id) {
        return service.findById(id)
            .map(ResponseEntity::ok)
            .orElseGet(() -> ResponseEntity.notFound().build());
    }
}
