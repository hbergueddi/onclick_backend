...
import com.onesley.oneclick.service.contract.ContractHistoryService;
...
/**
 * REST controller pour {@link ContractHistoryDto} (généré par scripts/scaffold-jpa.mjs).
 * Endpoints minimaux — étendre selon les besoins métier (filtres, pagination,
 * mutations, sécurité @PreAuthorize).
 */
@RestController
@RequestMapping("/api/contract-historys")
@Tag(name = "ContractHistory", description = "Auto-generated controller for contract_history")
public class ContractHistoryController {
    ...
    @GetMapping("/{id}")
    @Operation(summary = "Détail d'une ContractHistory par UUID")
    public ResponseEntity<ContractHistoryDto> findById(@PathVariable UUID id) {
