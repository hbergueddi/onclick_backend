...
- **P3 — Flip identité + révélation** : **BACKEND ✅ FAIT** (`a8712b8`, suite complète verte). **V95** flip générique clients programme → home `oneclick` (gardent membership). `GET /api/me/memberships` (`MeMembershipController`, source du reveal) ; `GET /api/tenants/{id}/members` (+ `/members/kpis`) ; **`MembershipKpiPublisher` STOMP** `/topic/admin/membership-kpis/{id}`. + nettoyage des scopings client-par-home cassés par le flip (addAdmin lookup global, PccStory ABAC membership). **FRONTEND ✅ VÉRIFIÉ** (tsc/vitest, 0 polling) mais **non committé** (intriqué WIP co-dev) → handoff `OneClick_Spring_FrontEnd/docs/P3-MEMBERSHIP-FRONTEND-HANDOFF.md`. **NATIF (P3.6) différé** → `docs/P3-MEMBERSHIP-NATIVE-HANDOFF.md`.
- **P4 — Nettoyage** : ✅ **FAIT** (`5e5f5ee`, suite complète verte + `verify()` 2/2). Scoping membre par home-tenant (caduc post-flip) migré vers `callerProgramTenant` dans feedback/seminar/membercircle/announcement (backward-compatible). `pcc_member_type` = vestigial (0 read logique ; source = `tenant_memberships.member_type`) → **DROP COLUMN différé** (éviter casse types FE générés) — documenté.
