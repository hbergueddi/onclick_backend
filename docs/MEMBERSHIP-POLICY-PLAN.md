...
- **P0 — Fondation (additif, iso‑comportement)** : Flyway `V90__tenant_memberships.sql` + module `membership` + port + **backfill miroir** (chaque CLIENT en tenant palmeraie/homu → membership active = son tenant, `member_type` repris ; sélection par **slug**, pas d'UUID). *Tests* : unit `MembershipService` + intégration (port + RBAC controller). **← EN COURS**
- **P1 — Authorities + ABAC via membership** (iso‑comportement, miroir) : pliage authorities + bascule scoping programme vers le port + `/me.memberships[]`. *Tests* : unit (pliage, ABAC) + intégration matrice membre 200 / non‑membre 403.
- **P2 — Invitation admin tenant** : `POST /api/tenants/{tenantId}/members` `hasAuthority('CREATE:MEMBERSHIPS')` + ABAC ; réutilise/crée le compte OneClick + membership ; email brandé via event (Resend mocké). *Tests* : unit (existant vs nouveau, idempotent) + intégration (admin 200 / non‑admin 403 / cross‑tenant 403).
